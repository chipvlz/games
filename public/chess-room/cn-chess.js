//旧浏览器不支持渐变，悔棋，和棋，吃自己的子为该提自己的子，微信上清空不了背景，棋子字的位置垂直方向不在中间
	...
	function select(event) {
		...
		function piecego(event) {
			setTimeout(function(){
				socket.emit('play', userId, roomId, getBoard() );
	...
	function piecedown(iizi,deadpiece) {
		//更改显示信息
		if (step===0) {
			startTxt.nodeValue="GAME MỚI";
			tipTxt.nodeValue="ĐANG TRONG GAME";
